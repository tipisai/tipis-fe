export enum TextSignal {
  PING = 0,
  ENTER = 1,
  LEAVE = 2,
  CREATE_STATE = 3,
  DELETE_STATE = 4,
  UPDATE_STATE = 5,
  MOVE_STATE = 6,
  CREATE_OR_UPDATE_STATE = 7,
  BROADCAST_ONLY = 8,
  PUT_STATE = 9,
  GLOBAL_BROADCAST_ONLY = 10,
  COOPERATE_ATTACH = 11,
  COOPERATE_DISATTACH = 12,
  MOVE_CURSOR = 13,
  FORCE_REFRESH = 14,
  TAKE_APP_SNAPSHOT = 15,
  RECOVER_APP_SNAPSHOT = 16,
  RUN = 17,
  STOP_RUN = 18,
  CLEAN_CHAT_HISTORY = 19,
  ADD_CHAT_FILE = 20,
  RECOVER_HISTORY_MESSAGE = 22,
}

export enum TextTarget {
  NOTHING = 0,
  COMPONENTS = 1,
  DEPENDENCIES = 2,
  DRAG_SHADOW = 3,
  DOTTED_LINE_SQUARE = 4,
  DISPLAY_NAME = 5,
  APPS = 6,
  RESOURCE = 7,
  ACTION = 8,
  CURSOR = 9,
  WINDOW = 10,
}

export enum WEBSOCKET_ERROR_CODE {
  ERROR_CODE_OK = 0,
  ERROR_CODE_FAILED = 1,
  ERROR_CODE_NEED_ENTER = 2,
  ERROR_CODE_PONG = 3,
  ERROR_CODE_LOGIN_FAILED = 4,
  ERROR_CREATE_STATE_FAILED = 5,
  ERROR_DELETE_STATE_FAILED = 6,
  ERROR_UPDATE_STATE_FAILED = 7,
  ERROR_MOVE_STATE_FAILED = 8,
  ERROR_CREATE_OR_UPDATE_STATE_FAILED = 9,
  ERROR_CAN_NOT_MOVE_KVSTATE = 10,
  ERROR_CAN_NOT_MOVE_SETSTATE = 11,
  ERROR_CREATE_SNAPSHOT_MODIFY_HISTORY_FAILED = 12,
  ERROR_UPDATE_SNAPSHOT_MODIFY_HISTORY_FAILED = 13,
  ERROR_FORCE_REFRESH_WINDOW = 14,
  ERROR_MESSAGE_END = 15,
  CONTEXT_LENGTH_EXCEEDED = 16,
  INSUFFICIENT_CREDIT = 17,
  AI_AGENT_MAX_TOKEN_OVER_CREDIT_BALANCE = 18,
  FLAG_ONLY_PAID_TEAM_CAN_RUN_SPECIAL_AI_AGENT_MODEL = 19,
  PUT_STATE_FAILED = 20,
  CHAT_FILE_PROCESS_END = 21,
  HISTORY_MESSAGE = 22,
}
